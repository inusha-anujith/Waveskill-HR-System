        ...
        {request.status === 'Pending' ? (
          <div className="flex gap-3 mt-4">
            <button 
              ...
              className="flex-1 py-3.5 bg-white border border-red-200 text-red-600 hover:bg-red-50 font-semibold rounded-xl transition-colors flex items-center justify-center gap-2 text-sm"
            >
              <XCircle size={18} /> Reject
            </button>
            <button 
              onClick={() => { onUpdateStatus(request.id, 'Approved'); onClose(); }}
              className="flex-1 py-3.5 bg-[#1a1a1a] text-white hover:bg-black font-semibold rounded-xl transition-colors flex items-center justify-center gap-2 text-sm"
            >
